...
interface DnsMonitorOperations {
    ...
    @Post("/")
    fun createMonitor(@Body monitor: DnsMonitorCreateDto): DnsMonitorDto
    ...
    @Operation(
        ...
        requestBody = RequestBody(content = [Content(schema = Schema(implementation = DnsMonitorUpdateDto::class))])
    )
    @Patch("/{monitorId}")
    fun updateMonitor(monitorId: Long, @Body updates: ObjectNode): DnsMonitorDto

    @Operation(summary = "Get the uptime events of the given monitor")
    @Get("/{monitorId}/uptime-events")
    fun getUptimeEvents(monitorId: Long): List<DnsUptimeEventDto>
