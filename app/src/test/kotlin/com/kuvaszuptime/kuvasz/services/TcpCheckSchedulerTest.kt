...
class TcpCheckSchedulerTest(
    ...
    init {
        given("the TcpCheckScheduler service") {
            ...
            `when`("there is an enabled but unschedulable monitor in the database and initialize has been called") {
                createTcpMonitor(monitorRepository, uptimeCheckInterval = 0)
            ...
            `when`("the getNextCheck() method is called, but no check is scheduled for the given monitor") {
                val monitor = createTcpMonitor(monitorRepository, uptimeCheckInterval = 10)
                checkScheduler.initialize()
            ...
            `when`("the getNextCheck() method is called, and there are scheduled checks for the monitor") {
                val monitor = createTcpMonitor(monitorRepository, uptimeCheckInterval = 100)
                checkScheduler.initialize()
    ...
    @MockBean(TcpUptimeChecker::class)
    fun uptimeCheckerMock(): TcpUptimeChecker = mockk()
