 ...
 */
class AppBootstrappingTcpMonitorYamlConfigTest : StringSpec({

    var appContext: ApplicationContext? = null

    val monitorsAfterTheFirstStep = mutableListOf<TcpMonitorRecord>()
    val monitorsAfterTheSecondStep = mutableListOf<TcpMonitorRecord>()
    ...
    fun getCheckScheduler() = appContext?.getBean<TcpCheckScheduler>().shouldNotBeNull()
    fun getMonitorRepository() = appContext?.getBean<TcpMonitorRepository>().shouldNotBeNull()
    fun getAppConfig() = appContext?.getBean<AppConfig>().shouldNotBeNull()
    fun getMonitorActions() = appContext?.getBean<TcpMonitorActions>().shouldNotBeNull()
    ...
    fun executeAndAssertTheFirstStep() {
        appContext = testAppContext("yaml-tcp-monitors", "full-integrations-setup")
        ...
        monitorsInDb.forOne { secondMonitor ->
            ...
            secondMonitor.enabled shouldBe TcpMonitorDefaults.MONITOR_ENABLED
            secondMonitor.timeoutMs shouldBe TcpMonitorDefaults.TIMEOUT_MS
            secondMonitor.latencyThresholdMs.shouldBeNull()
            secondMonitor.failureCountThreshold shouldBe TcpMonitorDefaults.FAILURE_COUNT_THRESHOLD
        ...
        monitorsInDb.forOne { thirdMonitor ->
            ...
            thirdMonitor.timeoutMs shouldBe TcpMonitorDefaults.TIMEOUT_MS
    ...
    "2. step: the app is restarted with some changes to the YAML configs" {
        ...
        appContext = testAppContext("yaml-tcp-monitors-changed", "full-integrations-setup")
        val checkScheduler = getCheckScheduler()
        val monitorRepository = getMonitorRepository()
        ...
        monitorsInDb.forOne { firstMonitor ->
            firstMonitor.name shouldBe "test1"
            firstMonitor.host shouldBe "127.0.0.1"
            firstMonitor.port shouldBe 5433
        ...
        monitorsInDb.forOne { thirdMonitor ->
            ...
            thirdMonitor.port shouldBe 8080
    ...
    "3. step: the app is restarted with an empty value in the YAML config for the monitors" {
        appContext = testAppContext("yaml-tcp-monitors-empty-value")
        val checkScheduler = getCheckScheduler()
        val monitorRepository = getMonitorRepository()

        // The app config should be set to enable external writes against the monitors
        getAppConfig().isTcpMonitorExternalWriteDisabled() shouldBe false
    ...
    "4. step: the app is restarted with no YAML config for TCP monitors" {
    ...
    "6. step: the app is started with an empty array in the YAML config for the monitors" {
        appContext = testAppContext("yaml-tcp-monitors-empty-array")
        val checkScheduler = getCheckScheduler()
        val monitorRepository = getMonitorRepository()
    ...
    "7. step: the app started with some monitors in the YAML, but there is a non-existing integration on one of them" {
        val ex = shouldThrow<BeanInstantiationException> {
            testAppContext("yaml-tcp-monitors-missing-integration", "full-integrations-setup")
        }
