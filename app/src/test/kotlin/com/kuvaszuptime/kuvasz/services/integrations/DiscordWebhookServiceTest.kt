...
class DiscordWebhookServiceTest(
    ...
    context("sendTestMessage") {
        ...
        should("call the webhook client to send the message") {
            ...
            val expectedMessage = DiscordWebhookMessage(content = Messages.integrationTestMessage())
            val config = mockk<DiscordNotificationConfig>(relaxed = true) {
            ...
            val result = discordWebhookService.sendTestMessage(config).blockingGet()
    ...
    @MockBean(DiscordWebhookClient::class)
    fun discordWebhookClient(): DiscordWebhookClient = mockk()
}
