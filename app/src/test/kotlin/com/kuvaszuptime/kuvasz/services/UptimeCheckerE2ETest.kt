    ...
    init {
        ...
        given("the UptimeChecker service") {
            ...
            `when`("a monitor's maintenance window is active and then becomes inactive") {
                val monitor = createHttpMonitor(
                    repository = monitorRepository,
                    url = "$mockServerUrl/maintenance-path",
                    requestMethod = HttpMethod.GET,
                ...
                val request = getRequest("/maintenance-path")
                mockServer.`when`(request).respond(
                    response().withStatusCode(HttpStatus.OK.code)
                )
                ...
                then("no check runs while the window is active, but checks resume once it becomes inactive") {
                    // While the window is active, the scheduled checks are skipped: no request reaches the target
                    delay(2500.milliseconds)
                    mockServer.verifyRequest(request, exactly = 0)
                    subscriber.assertNoValues()
                    ...
                    // The checks should run again and reach the target
                    val expectedEvent = subscriber.awaitCount(1).values().first()
                    expectedEvent.status shouldBe HttpStatus.OK
                    expectedEvent.monitor.id shouldBe monitor.id
