...
@MicronautTest(environments = ["yaml-tcp-monitors", "full-integrations-setup"])
class TcpMonitorMutabilityTest(
    @param:Client("/") private val client: HttpClient,
) : DatabaseStringSpec({

    val monitorCreateDto = TcpMonitorCreateDto(
        name = "something",
        host = "127.0.0.1",
        port = 8080,
        uptimeCheckInterval = 5149,
    )
    val monitorUpdateDto = JsonNodeFactory.instance.objectNode().put(TcpMonitorUpdateDto::enabled.name, false)
    ...
    "all the API endpoints that mutate monitors should return a 405 if the monitors are configured via YAML" {
        ...
        table(
            ...
            row("/api/v2/tcp-monitors", HttpMethod.POST, monitorCreateDto),
            row("/api/v2/tcp-monitors/1", HttpMethod.DELETE, null),
            row("/api/v2/tcp-monitors/1", HttpMethod.PATCH, monitorUpdateDto),
