...
@MicronautTest(environments = ["yaml-dns-monitors", "full-integrations-setup"])
class DnsMonitorMutabilityTest(
    @param:Client("/") private val client: HttpClient,
) : DatabaseStringSpec({

    val monitorCreateDto = DnsMonitorCreateDto(
        name = "something",
        host = "127.0.0.1",
        uptimeCheckInterval = 5149,
    )
    val monitorUpdateDto = JsonNodeFactory.instance.objectNode().put(DnsMonitorUpdateDto::enabled.name, false)
    ...
    "all the API endpoints that mutate monitors should return a 405 if the monitors are configured via YAML" {
        ...
        table(
            ...
            row("/api/v2/dns-monitors", HttpMethod.POST, monitorCreateDto),
            row("/api/v2/dns-monitors/1", HttpMethod.DELETE, null),
            row("/api/v2/dns-monitors/1", HttpMethod.PATCH, monitorUpdateDto),
