...
@MicronautTest(environments = ["yaml-docker-monitors", "full-integrations-setup", "docker-hosts"])
class DockerMonitorMutabilityTest(
    @param:Client("/") private val client: HttpClient,
) : DatabaseStringSpec({

    val monitorCreateDto = DockerMonitorCreateDto(
        name = "something",
        dockerHost = "local",
        container = "my-app",
        uptimeCheckInterval = 5149,
    )
    val monitorUpdateDto = JsonNodeFactory.instance.objectNode().put(DockerMonitorUpdateDto::enabled.name, false)
    ...
    "all the API endpoints that mutate monitors should return a 405 if the monitors are configured via YAML" {
        ...
        table(
            ...
            row("/api/v2/docker-monitors", HttpMethod.POST, monitorCreateDto),
            row("/api/v2/docker-monitors/1", HttpMethod.DELETE, null),
            row("/api/v2/docker-monitors/1", HttpMethod.PATCH, monitorUpdateDto),
