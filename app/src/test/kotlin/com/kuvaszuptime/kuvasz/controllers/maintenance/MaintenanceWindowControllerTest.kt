...
class MaintenanceWindowControllerTest(
    ...
    given("the createMaintenanceWindow endpoint") {
        ...
        `when`("a cron window is created without a duration") {
            then("it is rejected with a 400") {
                val response = shouldThrow<HttpClientResponseException> {
                    client.createMaintenanceWindow(MaintenanceWindowCreateDto(name = "No duration", cron = "0 2 * * *"))
                }
                response.status shouldBe HttpStatus.BAD_REQUEST
            }
        }
    ...
    given("the getMaintenanceWindow(s) endpoints") {
        ...
        `when`("windows exist") {
            createMaintenanceWindow(dsl, name = "Window A")
            val windowB = createMaintenanceWindow(dsl, name = "Window B")
    ...
    given("the deleteMaintenanceWindow endpoint") {
        ...
        `when`("it is called with an existing window ID") {
            val window = createMaintenanceWindow(dsl, name = "To delete")
    ...
    given("the updateMaintenanceWindow endpoint") {
        ...
        `when`("a manual window is converted into a cron window with all values updated") {
            val window = createMaintenanceWindow(dsl, name = "Manual", description = "before")
        ...
        `when`("a window is disabled via a partial update") {
            val window = createMaintenanceWindow(dsl, name = "Enabled", enabled = true)
            val updateDto = JsonNodeFactory.instance.objectNode().put(MaintenanceWindowUpdateDto::enabled.name, false)
        ...
        `when`("it is renamed to a name that already exists") {
            createMaintenanceWindow(dsl, name = "Existing name")
            val window = createMaintenanceWindow(dsl, name = "Original name")
            val updateDto = JsonNodeFactory.instance.objectNode()
        ...
        `when`("it is called with a blank name") {
            val window = createMaintenanceWindow(dsl, name = "Original")
        ...
        `when`("it is called with a null on a non-nullable property") {
            val window = createMaintenanceWindow(dsl, name = "Original", enabled = true)
        ...
        `when`("it is called with an invalid cron expression") {
            val window = createMaintenanceWindow(dsl, name = "Cron", cron = "0 2 * * *", duration = "PT1H")
            val updateDto = JsonNodeFactory.instance.objectNode().put(MaintenanceWindowUpdateDto::cron.name, "bad cron")
    ...
    given("the getYamlMaintenanceWindowsExport endpoint") {
        ...
        `when`("there are windows in the database") {
            ...
            createMaintenanceWindow(dsl, name = "Manual window", global = true)
            ...
            then("it exports them in YAML format") {
                ...
                val parsed = yamlMapper.convertValue<List<MaintenanceWindowExportDto>>(exportedRaw).shouldNotBeEmpty()
