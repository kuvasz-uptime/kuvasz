package com.kuvaszuptime.kuvasz.ui

internal enum class CSSClass(val className: String) {
    ACCORDION("accordion"),
    ACCORDION_BODY("accordion-body"),
    ACCORDION_BUTTON("accordion-button"),
    ACCORDION_BUTTON_TOGGLE("accordion-button-toggle"),
    ACCORDION_COLLAPSE("accordion-collapse"),
    ACCORDION_FLUSH("accordion-flush"),
    ACCORDION_HEADER("accordion-header"),
    ACCORDION_ITEM("accordion-item"),
    ACCORDION_TABS("accordion-tabs"),
    ALERT("alert"),
    ALERT_DANGER("alert-danger"),
    ALERT_DESCRIPTION("alert-description"),
    ALERT_DISMISSIBLE("alert-dismissible"),
    ALERT_HEADING("alert-heading"),
    ALERT_ICON("alert-icon"),
    ALERT_IMPORTANT("alert-important"),
    ALERT_INFO("alert-info"),
    ALERT_SUCCESS("alert-success"),
    ALERT_WARNING("alert-warning"),
    ALIGN_ITEMS_BASELINE("align-items-baseline"),
    ALIGN_ITEMS_CENTER("align-items-center"),
    ALIGN_MIDDLE("align-middle"),
    AVATAR("avatar"),
    BADGE("badge"),
    BADGE_LG("badge-lg"),
    BADGE_LIST("badge-list"),
    BADGE_PILL("badge-pill"),
    BADGE_SM("badge-sm"),
    BG_BLUE("bg-blue"),
    BG_BLUE_LT("bg-blue-lt"),
    BG_BODY("bg-body"),
    BG_CYAN("bg-cyan"),
    BG_CYAN_LT("bg-cyan-lt"),
    BG_DANGER("bg-danger"),
    BG_DEFAULT("bg-default"),
    BG_GRAY_300("bg-gray-300"),
    BG_GRAY_700("bg-gray-700"),
    BG_GREEN("bg-green"),
    BG_GREEN_LT("bg-green-lt"),
    BG_OPACITY_75("bg-opacity-75"),
    BG_ORANGE("bg-orange"),
    BG_ORANGE_LT("bg-orange-lt"),
    BG_PRIMARY("bg-primary"),
    BG_PURPLE_LT("bg-purple-lt"),
    BG_TEAL_LT("bg-teal-lt"),
    BG_RED_LT("bg-red-lt"),
    BG_SECONDARY("bg-secondary"),
    BG_SUCCESS("bg-success"),
    BG_SURFACE_BACKDROP("bg-surface-backdrop"),
    BG_WARNING("bg-warning"),
    BG_YELLOW("bg-yellow"),
    BG_YELLOW_LT("bg-yellow-lt"),
    BORDER_SECONDARY("border-secondary"),
    BORDER_TOP("border-top"),
    BOTTOM_0("bottom-0"),
    BTN("btn"),
    BTN_ACTIONS("btn-actions"),
    BTN_CLOSE("btn-close"),
    BTN_DANGER("btn-danger"),
    BTN_GHOST("btn-ghost"),
    BTN_GROUP("btn-group"),
    BTN_GROUP_SM("btn-group-sm"),
    BTN_ICON("btn-icon"),
    BTN_LINK("btn-link"),
    BTN_LIST("btn-list"),
    BTN_OUTLINE_DANGER("btn-outline-danger"),
    BTN_PRIMARY("btn-primary"),
    BTN_SECONDARY("btn-secondary"),
    BTN_SM("btn-sm"),
    CARD("card"),
    CARD_ACTIONS("card-actions"),
    CARD_BODY("card-body"),
    CARD_GRADIENT("card-gradient"),
    CARD_GRADIENT_GREEN("card-gradient-green"),
    CARD_GRADIENT_RED("card-gradient-red"),
    CARD_GRADIENT_SECONDARY("card-gradient-secondary"),
    CARD_GRADIENT_YELLOW("card-gradient-yellow"),
    CARD_HEADER("card-header"),
    CARD_LINK("card-link"),
    CARD_LINK_POP("card-link-pop"),
    CARD_SM("card-sm"),
    CARD_STAMP("card-stamp"),
    CARD_STAMP_ICON("card-stamp-icon"),
    CARD_STATUS_START("card-status-start"),
    CARD_SUBTITLE("card-subtitle"),
    CARD_TABLE("card-table"),
    CATEGORY_SECTION("category-section"),
    CARD_TITLE("card-title"),
    COL("col"),
    COLLAPSE("collapse"),
    COLLAPSED("collapsed"),
    COL_12("col-12"),
    COL_4("col-4"),
    COL_6("col-6"),
    COL_AUTO("col-auto"),
    COL_LG_3("col-lg-3"),
    COL_LG_4("col-lg-4"),
    COL_LG_6("col-lg-6"),
    COL_LG_AUTO("col-lg-auto"),
    COL_MD_2("col-md-2"),
    COL_MD_3("col-md-3"),
    COL_MD_4("col-md-4"),
    COL_MD_6("col-md-6"),
    COL_MD_AUTO("col-md-auto"),
    COL_SM_3("col-sm-3"),
    COL_SM_4("col-sm-4"),
    COL_SM_6("col-sm-6"),
    COL_SM_AUTO("col-sm-auto"),
    COL_XL_3("col-xl-3"),
    COL_XL_4("col-xl-4"),
    CONTAINER("container"),
    CONTAINER_XL("container-xl"),
    DISABLED("disabled"),
    DIVIDE_Y("divide-y"),
    DROPDOWN("dropdown"),
    DROPDOWN_DIVIDER("dropdown-divider"),
    DROPDOWN_ITEM("dropdown-item"),
    DROPDOWN_MENU("dropdown-menu"),
    DROPDOWN_TOGGLE("dropdown-toggle"),
    D_BLOCK("d-block"),
    D_FLEX("d-flex"),
    D_INLINE_FLEX("d-inline-flex"),
    D_LG_INLINE_BLOCK("d-lg-inline-block"),
    D_LG_TABLE_CELL("d-lg-table-cell"),
    D_MD_BLOCK("d-md-block"),
    D_MD_FLEX("d-md-flex"),
    D_MD_INLINE("d-md-inline"),
    D_MD_NONE("d-md-none"),
    D_MD_TABLE_CELL("d-md-table-cell"),
    D_NONE("d-none"),
    D_NONE_NAVBAR_HORIZONTAL("d-none-navbar-horizontal"),
    D_SM_TABLE_CELL("d-sm-table-cell"),
    EMPTY("empty"),
    EMPTY_ICON("empty-icon"),
    EMPTY_SUBTITLE("empty-subtitle"),
    EMPTY_TITLE("empty-title"),
    END_0("end-0"),
    FLEX_COLUMN("flex-column"),
    FLEX_FILL("flex-fill"),
    FLEX_MD_ROW("flex-md-row"),
    FLEX_NOWRAP("flex-nowrap"),
    FLEX_ROW("flex-row"),
    FLEX_WRAP("flex-wrap"),
    FLEX_ROW_REVERSE("flex-row-reverse"),
    FOOTER("footer"),
    FOOTER_TRANSPARENT("footer-transparent"),
    FORM_CHECK("form-check"),
    FORM_CHECK_INLINE("form-check-inline"),
    FORM_CHECK_INPUT("form-check-input"),
    FORM_CHECK_LABEL("form-check-label"),
    FORM_CHECK_SINGLE("form-check-single"),
    FORM_CONTROL("form-control"),
    FORM_CONTROL_SM("form-control-sm"),
    FORM_FOOTER("form-footer"),
    FORM_LABEL("form-label"),
    FORM_SELECT("form-select"),
    FORM_SELECT_SM("form-select-sm"),
    FORM_SELECTGROUP("form-selectgroup"),
    FORM_SELECTGROUP_INPUT("form-selectgroup-input"),
    FORM_SELECTGROUP_ITEM("form-selectgroup-item"),
    FORM_SELECTGROUP_LABEL("form-selectgroup-label"),
    FORM_SWITCH("form-switch"),
    FW_BOLDER("fw-bolder"),
    FW_MEDIUM("fw-medium"),
    G_2("g-2"),
    G_3("g-3"),
    H1("h1"),
    H2("h2"),
    H3("h3"),
    H4("h4"),
    H5("h5"),
    H6("h6"),
    HIDE_THEME_DARK("hide-theme-dark"),
    HIDE_THEME_LIGHT("hide-theme-light"),
    HR_TEXT("hr-text"),
    HTMX_INDICATOR("htmx-indicator"),
    ICON_INLINE("icon-inline"),
    ICON_LG("icon-lg"),
    IMG_PREVIEW("img-preview"),
    INPUT_ICON("input-icon"),
    INPUT_ICON_ADDON("input-icon-addon"),
    INVALID_FEEDBACK("invalid-feedback"),
    IS_INVALID("is-invalid"),
    JUSTIFY_CONTENT_CENTER("justify-content-center"),
    LH_1("lh-1"),
    LINK_SECONDARY("link-secondary"),
    LIST_INLINE("list-inline"),
    LIST_INLINE_DOTS("list-inline-dots"),
    LIST_INLINE_ITEM("list-inline-item"),
    LIST_UNSTYLED("list-unstyled"),
    LOGIN_CARD("login-card"),
    MB_0("mb-0"),
    MB_2("mb-2"),
    MB_3("mb-3"),
    MB_4("mb-4"),
    MB_6("mb-6"),
    ME_2("me-2"),
    ME_3("me-3"),
    ME_AUTO("me-auto"),
    MODAL("modal"),
    MODAL_BLUR("modal-blur"),
    MODAL_BODY("modal-body"),
    MODAL_CONTENT("modal-content"),
    MODAL_DIALOG("modal-dialog"),
    MODAL_DIALOG_CENTERED("modal-dialog-centered"),
    MODAL_FOOTER("modal-footer"),
    MODAL_HEADER("modal-header"),
    MODAL_LG("modal-lg"),
    MODAL_SM("modal-sm"),
    MODAL_STATUS("modal-status"),
    MODAL_TITLE("modal-title"),
    MS_1("ms-1"),
    MS_2("ms-2"),
    MS_AUTO("ms-auto"),
    MS_SM_AUTO("ms-sm-auto"),
    MS_LG_AUTO("ms-lg-auto"),
    MT_1("mt-1"),
    MT_2("mt-2"),
    MT_3("mt-3"),
    MT_4("mt-4"),
    MT_6("mt-6"),
    MT_LG_0("mt-lg-0"),
    MY_5("my-5"),
    M_0("m-0"),
    NATIVE_DATETIME_INPUT("native-datetime-input"),
    NAVBAR("navbar"),
    NAVBAR_BRAND("navbar-brand"),
    NAVBAR_BRAND_AUTODARK("navbar-brand-autodark"),
    NAVBAR_COLLAPSE("navbar-collapse"),
    NAVBAR_EXPAND_MD("navbar-expand-md"),
    NAVBAR_NAV("navbar-nav"),
    NAVBAR_TOGGLER("navbar-toggler"),
    NAVBAR_TOGGLER_ICON("navbar-toggler-icon"),
    NAV_ITEM("nav-item"),
    NAV_LINK("nav-link"),
    NAV_LINK_ICON("nav-link-icon"),
    NAV_LINK_TITLE("nav-link-title"),
    ORDER_MD_LAST("order-md-last"),
    PAGE("page"),
    PAGE_BODY("page-body"),
    PAGE_HEADER("page-header"),
    PAGE_PRETITLE("page-pretitle"),
    PAGE_TITLE("page-title"),
    PAGE_WRAPPER("page-wrapper"),
    PB_0("pb-0"),
    PB_3("pb-3"),
    PE_0("pe-0"),
    PE_MD_3("pe-md-3"),
    POSITION_ABSOLUTE("position-absolute"),
    POSITION_RELATIVE("position-relative"),
    PROGRESS("progress"),
    PROGRESS_BAR("progress-bar"),
    PROGRESS_SM("progress-sm"),
    PT_0("pt-0"),
    PX_0("px-0"),
    PX_3("px-3"),
    PY_2("py-2"),
    PY_4("py-4"),
    P_3("p-3"),
    REQUIRED("required"),
    ROUNDED("rounded"),
    ROW("row"),
    ROW_CARDS("row-cards"),
    SHOW("show"),
    SPINNER_GROW("spinner-grow"),
    SPINNER_GROW_SM("spinner-grow-sm"),
    START_0("start-0"),
    STATUS("status"),
    STATUS_CYAN("status-cyan"),
    STATUS_DOT("status-dot"),
    STATUS_DOT_ANIMATED("status-dot-animated"),
    STATUS_GRAY("status-gray"),
    STATUS_GREEN("status-green"),
    STATUS_INDICATOR("status-indicator"),
    STATUS_INDICATOR_ANIMATED("status-indicator-animated"),
    STATUS_INDICATOR_CIRCLE("status-indicator-circle"),
    STATUS_ORANGE("status-orange"),
    STATUS_RED("status-red"),
    STATUS_YELLOW("status-yellow"),
    STICKY_TOP("sticky-top"),
    SUBHEADER("subheader"),
    TABLE("table"),
    TABLE_RESPONSIVE("table-responsive"),
    TABLE_SM("table-sm"),
    TABLE_VCENTER("table-vcenter"),
    TEXT_AZURE("text-azure"),
    TEXT_BLUE("text-blue"),
    TEXT_BLUE_LT_FG("text-blue-lt-fg"),
    TEXT_BREAK("text-break"),
    TEXT_CENTER("text-center"),
    TEXT_CYAN("text-cyan"),
    TEXT_CYAN_LT_FG("text-cyan-lt-fg"),
    TEXT_DECORATION_NONE("text-decoration-none"),
    TEXT_DEFAULT_FG("text-default-fg"),
    TEXT_END("text-end"),
    TEXT_GREEN("text-green"),
    TEXT_GREEN_FG("text-green-fg"),
    TEXT_GREEN_LT_FG("text-green-lt-fg"),
    TEXT_INDIGO("text-indigo"),
    TEXT_MUTED("text-muted"),
    TEXT_NOWRAP("text-nowrap"),
    TEXT_ORANGE("text-orange"),
    TEXT_ORANGE_LT_FG("text-orange-lt-fg"),
    TEXT_PURPLE_LT_FG("text-purple-lt-fg"),
    TEXT_TEAL_LT_FG("text-teal-lt-fg"),
    TEXT_PRIMARY_FG("text-primary-fg"),
    TEXT_PURPLE("text-purple"),
    TEXT_RED("text-red"),
    TEXT_RED_LT_FG("text-red-lt-fg"),
    TEXT_RESET("text-reset"),
    TEXT_SECONDARY("text-secondary"),
    TEXT_TRUNCATE("text-truncate"),
    TEXT_UPPERCASE("text-uppercase"),
    TEXT_WRAP("text-wrap"),
    TEXT_YELLOW("text-yellow"),
    TEXT_YELLOW_FG("text-yellow-fg"),
    TEXT_YELLOW_LT_FG("text-yellow-lt-fg"),
    TOAST_CONTAINER("toast-container"),
    TOP_0("top-0"),
    TRACKING("tracking"),
    TRACKING_BLOCK("tracking-block"),
    W_25("w-25"),
    W_100("w-100"),
}
