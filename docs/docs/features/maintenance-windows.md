...
- **Uptime checks are paused.** HTTP, ICMP, TCP, and DNS checks are skipped entirely, SSL checks are postponed, and the scheduled heartbeat expiry check for push monitors is skipped. This means **no false incidents and no false alerts** are generated by the planned downtime.
...
!!!tip "Unplanned outages are a different problem"

    Maintenance windows are about downtime you **know about in advance**. If you want _Kuvasz_ to stop checking your monitors when **its own host** loses its network connection, that's what the [**connectivity check**](connectivity-check.md) does — using the very same "skip the check, record nothing, alert nobody" machinery.
