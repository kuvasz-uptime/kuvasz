...
=== "YAML (advanced)"
    ...
    **Restoring from a YAML backup**
    ...
    !!!warning "Restoring a backup is destructive"
        ...
        - Referenced monitors and integrations that no longer exist are **skipped** (with a warning) instead of failing the import, so import your monitors and configure your integrations **before** restoring your maintenance windows. A **malformed** monitor reference (not in the `<type>:<name>` format) is a different case and **fails** the whole import, so you can catch typos instead of silently dropping them.
